...
trap_dispatch(struct Trapframe *tf)
{
	...
	switch (tf->tf_trapno){
		case T_PGFLT:
		// currently those functions all drop to kernel monitor
		// which typically does not return.
		...
		case T_DEBUG:
			monitor(tf);
			return;
		// system call handling, 5 arguments are from saved registers
		case T_SYSCALL:{
			uint32_t syscallno = tf->tf_regs.reg_eax;
			// cprintf("[%x] in syscall [%d] on cpu [%d] status[%d]\n", curenv->env_id, tf->tf_regs.reg_eax, cpunum(), curenv->env_status);
			// use in_syscall to avoid running a env before syscall finishes
			if (xchg(&curenv->in_syscall, syscallno+10000))
				panic("env[%x] previous syscall not finished, status[%d] cpu[%d]", curenv->env_id, curenv->env_status, cpunum());
			tf->tf_regs.reg_eax = syscall(tf->tf_regs.reg_eax,
				tf->tf_regs.reg_edx, tf->tf_regs.reg_ecx, tf->tf_regs.reg_ebx,
				tf->tf_regs.reg_edi, tf->tf_regs.reg_esi);
			// cprintf("[%x] out syscall [%d] on cpu [%d] status[%d]\n", curenv->env_id, syscallno, cpunum(), curenv->env_status);
...
trap(struct Trapframe *tf)
{
	...
	if (curenv && curenv->env_status == ENV_RUNNABLE){
		cprintf("wtf..env_status of [%x] is runnable when trapped "
			"trapno[%d] eax[%d] on cpu[%d], previous on [%d]\n", curenv->env_id, tf->tf_trapno, tf->tf_regs.reg_eax, cpunum(), curenv->env_cpunum);
	}
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if ((tf->tf_cs & 3) == 0){
		print_trapframe(tf);
		uint32_t *bp = (uint32_t *)tf->tf_regs.reg_ebp;
		while (bp){
			cprintf("%x ", bp[1]);
			bp = (uint32_t *)bp[0];
		}
	...
	// LAB 4: Your code here.
	uint32_t user_mem_check_addr;
	spin_lock(&env_lock);
	if (user_mem_check(curenv, (void*)curenv->env_pgfault_upcall, 0,  0) < 0){
		user_mem_check_addr = (uint32_t)curenv->env_pgfault_upcall;
		goto page_fault_destroy_env;
	}
	if (user_mem_check(curenv, (void*)(UXSTACKTOP - PGSIZE), PGSIZE, PTE_W) < 0){
		user_mem_check_addr = (UXSTACKTOP - PGSIZE);
		goto page_fault_destroy_env;
	}
	// check if already on exception stack
	if (tf->tf_esp > UXSTACKTOP - PGSIZE){
		// check if about to overflow the excetion stack
		if (tf->tf_esp - 4 * 6 - sizeof(struct PushRegs) <= UXSTACKTOP - PGSIZE){
			cprintf("[%08x] user exception stack overflow!\n");
			env_destroy(curenv);
			return;
		}
		// reserve 4 bytes.
		exstack_addr = (uint32_t*)(tf->tf_esp - 4);
	}else exstack_addr = (uint32_t*)UXSTACKTOP;
	// get in user space
	lcr3(PADDR(curenv->env_pgdir));
	// push trap-time esp, eflags, saved registers etc.
	// to construct struct UTrapframe
	*(--exstack_addr) = tf->tf_esp;
	*(--exstack_addr) = tf->tf_eflags;
	*(--exstack_addr) = tf->tf_eip;
	exstack_addr -= sizeof(struct PushRegs)/sizeof(uint32_t);
	memcpy(exstack_addr, &(tf->tf_regs), sizeof(struct PushRegs));
	*(--exstack_addr) = tf->tf_err;
	*(--exstack_addr) = fault_va;
	// set sp, ip and drop to user program
	memset(&(tf->tf_regs), 0, sizeof(struct PushRegs));
	tf->tf_esp = (uint32_t)exstack_addr;
	tf->tf_eip = (uint32_t)curenv->env_pgfault_upcall;
	// does not return
	env_run(curenv);
	// Destroy the environment that caused the fault.
page_fault_destroy_env:
	cprintf("[%08x] user_mem_check assertion failure for "
			"va %08x, eip:0x%08x\n", curenv->env_id, user_mem_check_addr, tf->tf_eip);
	print_trapframe(tf);
	env_destroy(curenv);
}
